...
class JanelaRevisao(QDialog):
    def __init__(self, topico_id, nome_topico, parent=None):
        ...
        self.agendar = QCheckBox("Agendar próxima revisão")
        ...
        formulario.addRow("Data da revisão:", self.data)
        formulario.addRow("Questões:", self.questoes)
        formulario.addRow("Acertos:", self.acertos)
        formulario.addRow("Resultado:", self.resultado)
        formulario.addRow("", self.agendar)
        formulario.addRow("Próxima revisão:", self.proxima_revisao)
        ...
        self.agendar.toggled.connect(self.proxima_revisao.setEnabled)
    ...
    def atualizar_calculos(self):
        ...
        percentual = (
            self.acertos.value() / questoes
        ) * 100

        self.resultado.setText(
            formatar_percentual(percentual, 2)
        )

    def salvar(self):
        if self.agendar.isChecked():
            proxima = self.proxima_revisao.date().toString(
                "yyyy-MM-dd"
            )
        else:
            proxima = None
...
class JanelaTopico(QDialog):
    def __init__(self, topico_id, nome_topico, parent=None):
        ...
        layout.addWidget(self.aviso_legado)
    ...
    def carregar_historico(self):
        ...
        self.resumo.setText(
            f"Revisões totais: {resumo['revisoes_totais']}    |    "
            f"Resultado atual: {formatar_percentual(resumo['percentual_atual'])}    |    "
            f"Última revisão registrada: {formatar_data(resumo['ultima_revisao'])}    |    "
            f"Próxima revisão: {formatar_data(resumo['proxima_revisao'])}"
        )

        iniciais = resumo["revisoes_iniciais"]

        if iniciais > 0:
            self.aviso_legado.setText(
                f"{iniciais} revisão(ões) anteriores foram trazidas do Excel. "
                "A planilha não guardava os detalhes individuais dessas sessões; "
                "por isso elas entram no total, mas não aparecem como linhas no histórico abaixo."
            )
        else:
...
class SistemaEstudos(QMainWindow):
    ...
    def criar_cartao(self, titulo):
        ...
        rotulo.setStyleSheet("font-size: 12px;")

        valor = QLabel("0")
        valor.setStyleSheet("font-size: 24px; font-weight: bold;")
    ...
    def criar_tela_inicial(self):
        ...
        subtitulo.setStyleSheet("font-size: 15px;")
        ...
        cartao, self.card_hoje = self.criar_cartao("Revisões para hoje")
        grade.addWidget(cartao, 0, 0)

        cartao, self.card_atrasadas = self.criar_cartao("Revisões atrasadas")
        grade.addWidget(cartao, 0, 1)

        cartao, self.card_media = self.criar_cartao("Desempenho atual")
        grade.addWidget(cartao, 0, 2)

        cartao, self.card_revisoes = self.criar_cartao("Revisões registradas")
        grade.addWidget(cartao, 1, 0)

        cartao, self.card_questoes = self.criar_cartao("Questões no programa")
        grade.addWidget(cartao, 1, 1)

        cartao, self.card_topicos = self.criar_cartao("Tópicos cadastrados")
        grade.addWidget(cartao, 1, 2)

        layout.addLayout(grade)

        pendentes_titulo = QLabel("Revisões pendentes")
        pendentes_titulo.setStyleSheet(
            "font-size: 18px; font-weight: bold;"
        )
        layout.addWidget(pendentes_titulo)

        self.tabela_pendentes = QTableWidget()
        self.tabela_pendentes.setColumnCount(6)
        ...
        self.tabela_pendentes.setColumnWidth(0, 150)
        self.tabela_pendentes.setColumnWidth(2, 80)
        self.tabela_pendentes.setColumnWidth(3, 130)
        self.tabela_pendentes.setColumnWidth(4, 90)
        self.tabela_pendentes.setColumnWidth(5, 110)

        self.tabela_pendentes.cellDoubleClicked.connect(
            self.abrir_topico_pendente
        )

        layout.addWidget(self.tabela_pendentes, 1)

        disciplinas_titulo = QLabel("Disciplinas")
        disciplinas_titulo.setStyleSheet(
            "font-size: 18px; font-weight: bold;"
        )
        layout.addWidget(disciplinas_titulo)

        botoes_layout = QGridLayout()

        for indice, (_, nome) in enumerate(listar_disciplinas()):
    ...
    def atualizar_dashboard(self):
        ...
        pendencias = listar_pendencias(hoje)
        ...
        for linha, dado in enumerate(pendencias):
            topico_id = dado[0]
            disciplina = dado[1]
            topico = dado[2]
            revisoes = dado[3]
            ...
            data_pendente = QDate.fromString(
                proxima,
                "yyyy-MM-dd"
            )

            if data_pendente < data_hoje:
                situacao = "ATRASADA"
            else:
                situacao = "HOJE"
    ...
    def abrir_topico_pendente(self, linha, coluna):
    ...
    def criar_tela_disciplina(self):
        ...
        self.tabela_topicos.setColumnCount(5)
        ...
        self.tabela_topicos.setColumnWidth(1, 90)
        self.tabela_topicos.setColumnWidth(2, 135)
        self.tabela_topicos.setColumnWidth(3, 135)
        self.tabela_topicos.setColumnWidth(4, 90)
        ...
        for linha, dado in enumerate(dados):
            ...
            for coluna, valor in enumerate(valores):
                ...
                if coluna == 3 and proxima:
                    ...
                    if data_qt.isValid() and data_qt < hoje:
                        item.setBackground(
                            QColor("#FADBD8")
                        )
                    elif data_qt == hoje:
