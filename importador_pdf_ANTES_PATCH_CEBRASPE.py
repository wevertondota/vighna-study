...
PADRAO_GABARITO_CERTO_ERRADO = re.compile(
    r"""
    (?im)
    ^[ \t]*
    (?:GABARITO|RESPOSTA(?:[ \t]+CORRETA)?)
...
def _normalizar_formato_vpq(valor):
    valor = normalizar_texto_pdf(valor or "")
...
def _normalizar_gabarito_certo_errado(valor):
    valor = normalizar_texto_pdf(valor or "").strip()
    if valor in {"c", "certo"}:
        return "C"
...
def _analisar_texto_questoes_vpq_certo_errado(texto, faixas_paginas=None):
    texto = limpar_texto_pdf(texto)
    candidatos = [
        item
        for item in _candidatos_questao(texto)
    ...
    for indice, candidato in enumerate(candidatos):
        fim = (
            candidatos[indice + 1]["inicio"]
            if indice + 1 < len(candidatos)
            else len(texto)
        )
        bloco = texto[candidato["inicio"]:fim]
        prefixo = candidato["fim_prefixo"] - candidato["inicio"]
        corpo = bloco[prefixo:].strip()

        match_gabarito = PADRAO_GABARITO_CERTO_ERRADO.search(corpo)
        match_explicacao = PADRAO_EXPLICACAO.search(corpo)
        cortes = [m.start() for m in (match_gabarito, match_explicacao) if m]
        fim_enunciado = min(cortes) if cortes else len(corpo)
        enunciado = limpar_texto_pdf(corpo[:fim_enunciado]).strip()

        gabarito = (
            _normalizar_gabarito_certo_errado(match_gabarito.group("resposta"))
            if match_gabarito
            else None
        )
        ...
        questoes.append({
            "numero_pdf": int(candidato["numero"]),
            "enunciado": enunciado,
            "alternativas": [
                {"letra": "C", "texto": "Certo"},
            ...
            "gabarito": gabarito,
            "explicacao": explicacao,
            "pagina": _pagina_por_posicao(candidato["inicio"], faixas_paginas),
            "posicao": candidato["inicio"],
            "tipo_questao": "CERTO_ERRADO",
        })

    com_gabarito = sum(1 for item in questoes if item.get("gabarito"))
    return {
        "questoes": questoes,
        "gabarito_global": {},
        "texto_gabarito": "",
        ...
        "com_gabarito": com_gabarito,
        "sem_gabarito": len(questoes) - com_gabarito,
    }
...
def analisar_vpq_1_0(
    ...
    tipo_questao = _normalizar_formato_vpq(
        metadados.get("formato")
    )
    ...
    if tipo_questao == "CERTO_ERRADO":
        ...
        gabaritos_individuais = {
            int(item["numero_pdf"]): item["gabarito"]
    ...
    metadados["formato"] = tipo_questao
