...
class JanelaRevisao(QDialog):
    def __init__(
        ...
        if modo_edicao:
            self.setWindowTitle("Editar revisão")
        else:
        ...
        if modo_edicao and revisao is not None:
            ...
            self.questoes.setValue(revisao[2])
            self.acertos.setValue(revisao[3])
            self.observacao.setPlainText(revisao[5] or "")
            self.texto_erros.setPlainText(revisao[6] or "")
            self.continuacao.setPlainText(revisao[7] or "")

            # Ao editar uma revisão antiga, não alteramos a próxima
            # revisão automaticamente. Isso evita mudar a agenda atual
            # ao corrigir apenas um dado histórico.
            self.agendar.setChecked(False)
            self.agendar.setVisible(False)
            self.proxima_revisao.setVisible(False)
        else:
...
class JanelaTopico(QDialog):
    def __init__(
        ...
        alterar_data = QPushButton(
            "Alterar próxima revisão"
        )
        alterar_data.clicked.connect(
            self.alterar_proxima_revisao
        )

        barra.addWidget(alterar_data)
        barra.addStretch()

        self.editar_revisao_btn = QPushButton(
            "Editar revisão"
        )
        self.editar_revisao_btn.clicked.connect(
            self.editar_revisao
        )

        self.excluir_revisao_btn = QPushButton(
            "Excluir revisão"
        )
        self.excluir_revisao_btn.clicked.connect(
            self.excluir_revisao_selecionada
        )
        ...
        barra.addWidget(self.editar_revisao_btn)
        barra.addWidget(self.excluir_revisao_btn)
    ...
    def excluir_revisao_selecionada(self):
        ...
        resposta = QMessageBox.question(
            ...
            (
                ...
                f"Data: {formatar_data(revisao[1])}\n"
                f"Questões: {revisao[2]}\n"
                f"Acertos: {revisao[3]}\n"
                f"Resultado: {formatar_percentual(revisao[4])}\n\n"
                "Essa ação remove esse registro do histórico."
            ),
...
class SistemaEstudos(QMainWindow):
    ...
    def criar_tela_disciplina(self):
        ...
        barra.addWidget(renomear)
        barra.addWidget(excluir)
